...
<div class="wrap container-fluid">
	<h1 class="wp-heading-inline">Edit Data Penjualan</h1>
	<hr class="wp-header-end">
	<ul class="subsubsub">
		<li class="all"><a href="admin.php?page=konveksi-penjualan&pg=daftar">Daftar Penjualan</a> |</li>
		<li class="publish"><a href="admin.php?page=konveksi-penjualan" class="current" aria-current="page">Penjualan Sablon & Kaos</a> |</li>
  ...
  <form id="form-penjualan" action="admin.php?page=konveksi-penjualan" method="post">
    ...
    <input type="hidden" name="poscode" value="<?php echo $transaksi->pos_code; ?>">          
    ...
    <table class="wp-list-table widefat table-bordered table" width="100%">
      <thead>
        <tr>
          <th colspan="4">Informasi Penjualan</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td width="15%">Tanggal</td>
          <td width="35%"><input type="date" name="tanggal" value="<?php echo $transaksi->tanggal_transaksi; ?>" class="form-control form-control-sm"></td>
          ...
          <select id="channel" name="channel" class="mdb-select form-control form-control-sm">
              ...
              foreach ( $data['list-channel'] as $channel ) {
                  ...
                  if ( $transaksi->channel == $channel->jenis_generic_master ){ 
                    ?>
        ...
        <tr>
          <td>Pengambilan</td>
          <td><input type="date" name="pengambilan" value="<?php echo $transaksi->pengambilan; ?>" class="form-control form-control-sm"></td>
          <td>Segment</td>
          <td>
          <select id="segment" name="segment" class="mdb-select form-control form-control-sm">
              ...
              foreach ( $data['list-segmen'] as $segment ) {
                  ...
                  if ( $transaksi->segment != $segment->jenis_generic_master ){ 
                    ?>
                    <option value="<?php echo $segment->jenis_generic_master ?>" ><?php echo $segment->value_generic_master ?></option>
                    <?php
                  } else {
                    ?>
                    <option value="<?php echo $segment->jenis_generic_master ?>" SELECTED><?php echo $segment->value_generic_master ?></option>
                    <?php 
                  }
              } 
              ?>
        ...
        <tr>
          ...
          <select id="selUser" name="pelanggan" class="mdb-select form-control form-control-sm">
              ...
              foreach ( $data['list-pelanggan'] as $pelanggan ) {
                  ...
                  if ( $transaksi->no_pelanggan != $pelanggan->no_pelanggan ){ 
                    ?>
                    <option value="<?php echo $pelanggan->no_pelanggan ?>" ><?php echo $pelanggan->nama ?></option>
                    <?php
                  } else {
                    ?>
                    <option value="<?php echo $pelanggan->no_pelanggan ?>" SELECTED><?php echo $pelanggan->nama ?></option>
                    <?php 
                  }
              } 
              ?>
          ...
          <td>Ref. No</td>
          <td><input type="text" class="form-control form-control-sm" value="<?php echo $transaksi->no_transaksi_ref ; ?>" name="refno"></td>
        </tr>
        <tr>
          ...
          <td width="35%">
             <select id="akun" name="akun" class="mdb-select form-control form-control-sm">
              ...
              foreach ( $data['masterjurnal'] as $masterjurnal ) {
                  ...
                  if ( $transaksi->tipe != $masterjurnal->kode_masterjurnal ){ 
                    ?>
                    <option value="<?php echo $masterjurnal->kode_masterjurnal ?>" ><?php echo $masterjurnal->nama_masterjurnal ?></option>
                    <?php
                  } else {
                    ?>
                    <option value="<?php echo $masterjurnal->kode_masterjurnal ?>" SELECTED><?php echo $masterjurnal->nama_masterjurnal ?></option>
                    <?php 
                  }
              } 
              ?>
          ...
          <td>Order Name</td>
          <td><input type="text" class="form-control form-control-sm" value="<?php echo $transaksi->nama_order ; ?>" name="ordername"></td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td><input type="number" name="subtotal" id="SubTotal" value="<?php echo $transaksi->sub_total ; ?>" class="form-control form-control-sm"></td>
        </tr>
        <tr>
          <td colspan="3" width="65%" align="right"><b>Biaya Pengiriman</b></td>
          <td><input type="number" name="biayapengiriman" id="BiayaPengiriman" value="<?php echo $transaksi->biaya_pengiriman ; ?>" class="form-control form-control-sm"></td>
        </tr>
        <tr>
          <td colspan="3" width="65%" align="right"><b>Biaya Lain</b></td>
          <td><input type="number" name="biayalain" id="BiayaLain" value="<?php echo $transaksi->biaya_lain ; ?>" class="form-control form-control-sm"></td>
        </tr>
        <tr>
          <td colspan="3" width="65%" align="right"><b>Potongan</b></td>
          <td><input type="number" name="potongan" id="Potongan" value="<?php echo $transaksi->potongan ; ?>" class="form-control form-control-sm"></td>
        </tr>
        <tr>
          <td colspan="3" width="65%" align="right"><b>Total</b></td>
          <td><input type="number" name="total" id="Total" value="<?php echo $transaksi->total_akhir ; ?>" class="form-control form-control-sm"></td>
        </tr>
        <tr>
          <td colspan="3" width="65%" align="right"><b>Pembayaran</b></td>
          <td><input type="text" name="pembayaran" id="Pembayaran" value="<?php echo $transaksi->total_pembayaran ; ?>" class="form-control form-control-sm"></td>
        </tr>
        <tr>
        ...
        <tr>
          <td colspan="4">
            ...
            <input type="text" class="form-control form-control-sm" name="keterangan" value="<?php echo $transaksi->keterangan ; ?>" >
          </td>
        </tr>
        <tr>
          <td colspan="4">
            <label for="inputAddress">Catatan Produksi</label>
            <input type="text" class="form-control form-control-sm" name="catatan" value="<?php echo $transaksi->catatan_order ; ?>">
          </td>
        </tr>
        <?php } ?>
      </tbody>
    </table>
    <button type="submit" class="btn btn-primary" name="uptsubmit">Simpan</button>
  </form>     
...
var data = <?php echo json_encode($data['trx_detail']); ?>;
